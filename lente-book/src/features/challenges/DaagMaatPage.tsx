...
function InviteCreator() {
  ...
  const share = async () => {
    if (!invite) return
    try {
      if (navigator.share) await navigator.share({ title: 'Daag ’n Maat Uit', text: `${profile?.username} daag jou uit om ’n nuwe Afrikaanse woord te skep!`, url: inviteUrl })
