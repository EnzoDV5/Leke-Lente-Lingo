...
import styles from './RaaiDieLingoPage.module.css'
...
export default function RaaiDieLingoPage() {
  ...
  const allPhrases = useMemo(() => {
    const fallback = frases.map((phrase, index) => ({ id: phrase.id, text: phrase.beskrywing, area: phrase.area, boardNumber: index + 1, isWildcard: phrase.bord === 'Wildcard-skeppings' }))
  ...
  const candidates = useMemo(() => {
    const validIds = new Set(allPhrases.map((phrase) => phrase.id))
  ...
  const wordPhrase = word ? allPhrases.find((phrase) => phrase.id === word.phraseId) : null
  const phrasesList = useMemo(() => {
    if (!wordPhrase) return []
    return wordPhrase.isWildcard
      ? allPhrases.filter((phrase) => phrase.isWildcard)
      : allPhrases.filter((phrase) => !phrase.isWildcard && phrase.area === wordPhrase.area)
  }, [allPhrases, wordPhrase])
  ...
  const guess = async (phraseId: string) => {
    ...
    if (phraseId === word.phraseId) {
      setResult('correct')
      if (user) await completeChallenge(user.uid, 'guess', fromScan, { kind: 'guess', word: word.text, phrase: wordPhrase?.text ?? '', area: wordPhrase?.area ?? '', itemId: word.id })
  ...
  const moveChoices = (direction: -1 | 1) => choicesRef.current?.scrollBy({ left: direction * Math.min(560, choicesRef.current.clientWidth * .78), behavior: 'smooth' })

  if (wordsLoading || phrasesLoading) return <div className={styles.loading}><LoadingCard label="Jou raaisel word gebou…" /></div>
  ...
  return (
    <section className={styles.page}>
      ...
      <CompactHero className={styles.guessHero} kicker="03 · RAAI DIE LINGO" title="By watter frase hoort dié woord?" statement detail topAction={<Link to="/collections" className={styles.back}><span aria-hidden="true">←</span> My posters</Link>}>
      ...
      <div className={styles.wrap}>
        ...
        <div className={styles.carouselHint}><span>← Swiep deur die frases →</span><div><button type="button" onClick={() => moveChoices(-1)} aria-label="Vorige frases">‹</button><button type="button" onClick={() => moveChoices(1)} aria-label="Volgende frases">›</button></div></div>
        <div ref={choicesRef} className={styles.choices} aria-label="Frase-keuses">{phrasesList.map((phrase, index) => {
          ...
          return <button key={phrase.id} type="button" className={`${styles.choice} ${correct ? styles.correct : ''} ${wrong ? styles.wrong : ''}`} onClick={() => void guess(phrase.id)} disabled={result !== null}><span>0{index + 1}</span><div><small>{phrase.isWildcard ? 'Wildcard-skeppings' : AREA_NAMES[phrase.area]}</small><strong>{phrase.text}</strong></div><i>{correct ? '✓' : wrong ? '×' : 'KIES'}</i></button>
        })}</div>
        {result && <section className={`${styles.feedback} ${result === 'correct' ? styles.feedbackCorrect : styles.feedbackWrong}`} role="status"><span>{result === 'correct' ? '✓' : '×'}</span><div><strong>{result === 'correct' ? 'Reg geraai!' : 'Nie heeltemal nie.'}</strong><p>{result === 'correct' ? (fromScan ? 'Die Raai die Lingo-poster is nou versamel.' : 'Jy het die woord by sy frase gepas.') : 'Dit was jou een raai. Die regte frase is in groen gewys.'}</p></div>{result === 'wrong' && <button type="button" onClick={nextWord}>Probeer ’n nuwe woord</button>}</section>}
      </div>
    </section>
  )
}
