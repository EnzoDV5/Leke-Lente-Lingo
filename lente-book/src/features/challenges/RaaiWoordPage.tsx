...
export default function RaaiWoordPage() {
  ...
  return (
    <section className={styles.page}>
      ...
      <CompactHero className={styles.guessHero} kicker="03 · RAAI DIE WOORD" title="By watter frase hoort dié woord?" statement detail topAction={<Link to="/woordjag" className={styles.back}><span aria-hidden="true">←</span> My posters</Link>}>
      ...
      <div className={styles.wrap}>
        ...
        {result && <section className={`${styles.feedback} ${result === 'correct' ? styles.feedbackCorrect : styles.feedbackWrong}`} role="status"><span>{result === 'correct' ? '✓' : '×'}</span><div><strong>{result === 'correct' ? 'Reg geraai!' : 'Nie heeltemal nie.'}</strong><p>{result === 'correct' ? (fromScan ? 'Die Raai die Woord-poster is nou versamel.' : 'Jy het die woord by sy frase gepas.') : 'Dit was jou een raai. Die regte frase is in groen gewys.'}</p></div>{result === 'wrong' && <button type="button" onClick={nextWord}>Probeer ’n nuwe woord</button>}</section>}
      </div>
    </section>
  )
}
